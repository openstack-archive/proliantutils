...
def erase_devices():
    ...
    for physical_drive in physical_drives:
        physical_drive_dict = physical_drive.get_physical_drive_dict()
        if physical_drive_dict['erase_support'] == 'True':
            supported_drives.append(physical_drive)

    if not supported_drives:
        reason = ("No unassigned physical disks available to perform"
                  " the sanitize disk erase")
        raise exception.HPSSAException(reason)

    for drives in supported_drives:
        if physical_drive_dict['erase_status'] == 'OK':
            controller_id = drives.get_physical_drive_dict().get('controller')
            controller = server.get_controller_by_id(controller_id)
            controller.erase_devices(drives.id)

    erase_in_progress = True
    while(erase_in_progress):
        server.refresh()
        if any((drives.get_physical_drive_dict().get('erase_status', '')
                is not 'Erase In Progress')
               for drives in supported_drives):
