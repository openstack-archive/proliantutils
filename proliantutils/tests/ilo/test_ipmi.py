...
class IloIpmiTestCase(unittest.TestCase):
    ...
    @mock.patch.object(subprocess, 'check_output')
    def test__exec_ipmitool_device_not_present(self, check_mock):
        check_mock.return_value = "Device not present"
        expected_output = "Device not present"
        cmd = "fru print 0x64"
        actual_out = ipmi._exec_ipmitool(self.info, cmd)
        self.assertEqual(expected_output, actual_out)

    @mock.patch.object(subprocess, 'check_output')
    def test__exec_ipmitool_unknown_fru(self, check_mock):
        check_mock.return_value = "Unknown FRU header 0x64"
        expected_output = "Unknown FRU header 0x64"
        cmd = "fru print 0x64"
        actual_out = ipmi._exec_ipmitool(self.info, cmd)
        self.assertEqual(expected_output, actual_out)
