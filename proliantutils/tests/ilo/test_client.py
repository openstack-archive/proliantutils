...
class IloClientInitTestCase(testtools.TestCase):
    ...
    def test_init_for_ipv6(self, ris_mock, ribcl_mock):
        ...
        c = client.IloClient("FE80::9AF2:B3FF:FEEE:F884%eth0", "admin", "Admin",
                             timeout=120,  port=4430,
                             bios_password='foo',
                             cacert='/somewhere')

        ris_mock.assert_called_once_with(
            "[FE80::9AF2:B3FF:FEEE:F884%eth0]", "admin", "Admin", bios_password='foo',
            cacert='/somewhere')
        ribcl_mock.assert_called_once_with(
            "[FE80::9AF2:B3FF:FEEE:F884%eth0]", "admin", "Admin", 120, 4430, cacert='/somewhere')
        self.assertEqual(
            {'address': "[FE80::9AF2:B3FF:FEEE:F884%eth0]", 'username': "admin", 'password': "Admin"},
            c.info)
        self.assertEqual('product', c.model)
