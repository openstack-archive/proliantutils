...
class ProliantHardwareManagerTestCase(testtools.TestCase):
    ...
    def test_get_clean_steps(self):
        node = {'properties': {'device_erase_type': 'hardware'}}
        self.assertEqual(
    ...
    def test_get_clean_steps_without_erase_devices(self):
        node = {'properties': {'device_erase_type': 'shred'}}
        self.assertEqual(
            [{'step': 'create_configuration',
              'interface': 'raid',
              'priority': 0},
    ...
    def test__is_hardware_disk_erase_requested(self):
        node = {'properties': {'device_erase_type': 'hardware'}}
        ret = self.hardware_manager._is_hardware_disk_erase_requested(node)
        self.assertTrue(ret)

    def test__is_hardware_disk_erase_requested_shred(self):
        node = {'properties': {'device_erase_type': 'shred'}}
        ret = self.hardware_manager._is_hardware_disk_erase_requested(node)
