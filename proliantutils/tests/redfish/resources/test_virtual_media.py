...
class VirtualMediaCollectionTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        self.vmedia_inst = virtual_media.VirtualMediaCollection(
            self.conn, '/redfish/v1/Managers/1/VirtualMedia',
            redfish_version='1.0.2')

    def test_get_virtual_media_member_from_device(self):
        with open('proliantutils/tests/redfish/'
                  'json_samples/vmedia.json', 'r') as f:
            self.conn.get.return_value.json.return_value = (
                json.loads(f.read())['default'])

        obj = self.vmedia_inst.get_virtual_media_member_from_device('cdrom')
...
class VirtualMediaTestCase(testtools.TestCase):
    ...
    def test__parse_attributes(self):
        ...
        self.assertEqual(['cdrom', 'dvd'], self.vmedia_inst.media_types)
