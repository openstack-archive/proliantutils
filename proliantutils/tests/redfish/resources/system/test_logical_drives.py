...
class LogicalDriveTestCase(testtools.TestCase):

    def setUp(self):
        super(LogicalDriveTestCase, self).setUp()
        self.conn = mock.Mock()
        ...
        self.sys_stor = logical_drives.LogicalDrive(
...
class LogicalDriveCollectionTestCase(testtools.TestCase):

    def setUp(self):
        super(LogicalDriveCollectionTestCase, self).setUp()
        self.conn = mock.Mock()
        with open('proliantutils/tests/redfish/json_samples/'
                  'logical_drive_collection.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        self.sys_stor_col = logical_drives.LogicalDriveCollection(
    ...
    @mock.patch.object(logical_drives, 'LogicalDrive', autospec=True)
