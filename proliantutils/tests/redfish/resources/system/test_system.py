...
class HPESystemTestCase(testtools.TestCase):
    ...
    def test_memory_collection(self):
        self.assertIsNone(self.sys_inst._memory_collection)
        self.conn.get.return_value.json.reset_mock()
        with open('proliantutils/tests/redfish/'
                  'json_samples/memory_collection.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        actual_memory = self.sys_inst.memory_collection
        ...
        self.assertIs(actual_memory,
                      self.sys_inst.memory_collection)
        self.conn.get.return_value.json.assert_not_called()
    ...
    def test_memory_collection_on_refresh(self):
        ...
        with open('proliantutils/tests/redfish/json_samples/'
            ...
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        # | WHEN & THEN |
        self.assertIsInstance(self.sys_inst.memory_collection,
                              memory.MemoryCollection)
