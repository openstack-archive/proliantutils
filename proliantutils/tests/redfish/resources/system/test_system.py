...
class HPESystemTestCase(testtools.TestCase):
    ...
    def test__post_delete_read_raid(
        ...
        expected = [('HPE Smart Array P408i-a SR Gen10', config)]
        (get_smart_storage_config_mock.
         return_value.read_raid.return_value) = config
        self.assertEqual(expected, self.sys_inst._post_delete_read_raid())
        get_smart_storage_config_mock.assert_called_once_with(config_id[0])
    ...
    def test__post_create_read_raid(
        ...
        (get_ssc_by_controller_model_mock.
         return_value.read_raid.return_value) = config
        self.assertEqual(
            expected, self.sys_inst._post_create_read_raid(raid_config))
    ...
    def test_read_raid_post_create(
        ...
        config = {'logical_disks': result_ld1}
        expected = [('HPE Smart Array P408i-p SR Gen10', config)]
        post_create_read_raid_mock.return_value = expected
        self.assertEqual(
            expected, self.sys_inst.read_raid(raid_config=raid_config))
    ...
    def test_read_raid_post_delete(
        ...
        config = {'logical_disks': result_ld1}
        expected = [('HPE Smart Array P408i-p SR Gen10', config)]
        post_delete_read_raid_mock.return_value = expected
        self.assertEqual(
            expected, self.sys_inst.read_raid(raid_config=None))
