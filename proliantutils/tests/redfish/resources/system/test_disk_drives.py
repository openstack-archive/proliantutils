...
class DiskDriveTestCase(testtools.TestCase):

    def setUp(self):
        super(DiskDriveTestCase, self).setUp()
        self.conn = mock.Mock()
        ...
        self.sys_stor = disk_drives.DiskDrive(
...
class DiskDriveCollectionTestCase(testtools.TestCase):

    def setUp(self):
        super(DiskDriveCollectionTestCase, self).setUp()
        self.conn = mock.Mock()
        with open('proliantutils/tests/redfish/json_samples/'
                  'disk_drive_collection.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        self.sys_stor_col = disk_drives.DiskDriveCollection(
    ...
    @mock.patch.object(disk_drives, 'DiskDrive', autospec=True)
