...
class ArrayControllerTestCase(testtools.TestCase):
    ...
    def test_logical_drive(self):
        log_coll = None
        log_dr = None
        self.assertIsNone(self.sys_stor._logical_drive)
        ...
        actual_log_dr = self.sys_stor.logical_drive
        self.conn.get.return_value.json.reset_mock()
        self.assertIs(actual_log_dr,
                      self.sys_stor.logical_drive)
        self.conn.get.return_value.json.assert_not_called()

    def test_physical_drive(self):
        disk_coll = None
        disk_dr = None
        self.assertIsNone(self.sys_stor._physical_drive)
        self.conn.get.return_value.json.reset_mock()
        with open('proliantutils/tests/redfish/'
        ...
        actual_log_dr = self.sys_stor.physical_drive
        self.conn.get.return_value.json.reset_mock()
        self.assertIs(actual_log_dr,
                      self.sys_stor.physical_drive)
        self.conn.get.return_value.json.assert_not_called()

    def test_unconfigured_drive(self):
        disk_coll = None
        disk_dr = None
        self.assertIsNone(self.sys_stor._unconfigured_drive)
        self.conn.get.return_value.json.reset_mock()
        with open('proliantutils/tests/redfish/'
        ...
        with open('proliantutils/tests/redfish/'
            ...
            disk_dr = json.loads(f.read())
        self.conn.get.return_value.json.side_effect = [disk_coll, disk_dr]
        actual_log_dr = self.sys_stor.unconfigured_drive
        self.conn.get.return_value.json.reset_mock()
        self.assertIs(actual_log_dr,
                      self.sys_stor.unconfigured_drive)
        self.conn.get.return_value.json.assert_not_called()
