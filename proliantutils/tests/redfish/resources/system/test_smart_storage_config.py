...
class HPESmartStorageConfigTestCase(testtools.TestCase):
    ...
    def test_delete_raid(self):
        ...
        type(self.ssc_inst._conn.put.return_value).status_code = (
            mock.PropertyMock(return_value=200))
        result = self.ssc_inst.delete_raid()
    ...
    def test_delete_raid_logical_drive_not_found(self):
        type(self.ssc_inst).logical_drives = mock.PropertyMock(
            return_value=[])
        result = self.ssc_inst.delete_raid()
        self.assertEqual("Status: No Logical drives found!", result)
        self.ssc_inst._conn.put.assert_not_called()

    def test_delete_raid_response_failed(self):
        type(self.ssc_inst._conn.put.return_value).status_code = (
            mock.PropertyMock(return_value=404))
        self.assertRaisesRegex(
            exception.IloError,
            'Unable to delete raid',
