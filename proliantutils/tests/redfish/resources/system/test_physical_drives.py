...
class PhysicalDriveTestCase(testtools.TestCase):

    def setUp(self):
        super(PhysicalDriveTestCase, self).setUp()
        self.conn = mock.Mock()
        ...
        self.sys_stor = physical_drives.PhysicalDrive(
...
class PhysicalDriveCollectionTestCase(testtools.TestCase):

    def setUp(self):
        super(PhysicalDriveCollectionTestCase, self).setUp()
        self.conn = mock.Mock()
        with open('proliantutils/tests/redfish/json_samples/'
                  'disk_drive_collection.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        self.sys_stor_col = physical_drives.PhysicalDriveCollection(
    ...
    @mock.patch.object(physical_drives, 'PhysicalDrive', autospec=True)
