...
class StorageCollectionTestCase(testtools.TestCase):
    ...
    def test_volume_paths(self):
        self.assertIsNone(self.sys_stor_col._volume_paths)
        self.conn.get.return_value.json.reset_mock()
        path = ('proliantutils/tests/redfish/json_samples/'
                'storage1.json')
        with open(path, 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        expected = ["/redfish/v1/Systems/437XR1138R2/Storage/1/Volumes"]
        actual = self.sys_stor_col.volume_paths
        self.assertEqual(expected, actual)

    def test_volume(self):
        log_coll = None
        log_dr = None
        self.sys_stor_col._volume_paths = (
            ["/redfish/v1/Systems/437XR1138R2/Storage/1/Volumes"])
        self.assertIsNone(self.sys_stor_col._volume)
        self.conn.get.return_value.json.reset_mock()
        ...
        actual_log_dr = self.sys_stor_col.volume
        self.conn.get.return_value.json.reset_mock()
        self.assertIs(actual_log_dr,
                      self.sys_stor_col.volume)
        self.conn.get.return_value.json.assert_not_called()

    def test_disk_drive_paths(self):
        self.assertIsNone(self.sys_stor_col._disk_drive_paths)
        self.conn.get.return_value.json.reset_mock()
        path = ('proliantutils/tests/redfish/json_samples/'
                'storage1.json')
        with open(path, 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        expected = ["/redfish/v1/Systems/437XR1138R2/Storage/1/Drives/35D38F11ACEF7BD3",
                    "/redfish/v1/Systems/437XR1138R2/Storage/1/Drives/3F5A8C54207B7233"]
        actual = self.sys_stor_col.disk_drive_paths
        self.assertEqual(expected, actual)

    def test_disk_drive(self):
        dr1 = None
        dr2 = None
        self.sys_stor_col._disk_drive_paths = (
            ["/redfish/v1/Systems/437XR1138R2/Storage/1/Drives/35D38F11ACEF7BD3",
             "/redfish/v1/Systems/437XR1138R2/Storage/1/Drives/3F5A8C54207B7233"])
        self.assertIsNone(self.sys_stor_col._disk_drive)
        self.conn.get.return_value.json.reset_mock()
        with open('proliantutils/tests/redfish/'
        ...
        actual_dr = self.sys_stor_col.disk_drive
        self.conn.get.return_value.json.reset_mock()
        self.assertIs(actual_dr,
                      self.sys_stor_col.disk_drive)
        self.conn.get.return_value.json.assert_not_called()
