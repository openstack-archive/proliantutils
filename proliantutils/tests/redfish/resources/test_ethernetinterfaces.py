...
class EthernetInterfaceTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        self.conn = mock.Mock()
        eth_file = 'proliantutils/tests/redfish/json_samples/ethernetinterfaces.json'
        with open(eth_file, 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())

        eth_path = ("/redfish/v1/Systems/437XR1138R2/EthernetInterfaces/"
                    "12446A3B0411")
        self.sys_eth = ethernetinterface.EthernetInterface(
    ...
    def test__parse_attributes(self):
        ...
        self.assertEqual(1000, self.sys_eth.speedmbps)
...
class EthernetInterfaceCollectionTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        with open('proliantutils/tests/redfish/json_samples/'
                  'ethernetinterfaces_collection.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        self.sys_eth_col = ethernetinterface.EthernetInterfaceCollection(
    ...
    @mock.patch.object(ethernetinterface, 'EthernetInterface', autospec=True)
    ...
    def test_eth_summary(self):
        self.assertIsNone(self.sys_eth_col._eth_summary)
        self.conn.get.return_value.json.reset_mock()
        path = 'proliantutils/tests/redfish/json_samples/ethernetinterfaces.json'
        with open(path, 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        expected_summary = {'12:44:6A:3B:04:11': 'Enabled'}
        actual_summary = self.sys_eth_col.eth_summary
