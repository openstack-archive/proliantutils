...
class BIOSBootSettingsTestCase(testtools.TestCase):
    ...
    def test__get_persistent_boot_device(self):
        with open('proliantutils/tests/redfish/'
                  'json_samples/bios.json', 'r') as f:
            boot_json = (json.loads(f.read())['BIOS_boot_default'])
        self.bios_boot_inst.persistent_boot_config_order = (
        ...
        result = self.bios_boot_inst._get_persistent_boot_device()
        self.assertEqual(result, sushy.BOOT_SOURCE_TARGET_HDD)

    def test__get_persistent_boot_device_without_boot(self):
        with open('proliantutils/tests/redfish/'
                  'json_samples/bios.json', 'r') as f:
        ...
        self.assertRaisesRegex(
            exception.IloError,
            'Get persistent boot device failed with the error.',
            self.bios_boot_inst._get_persistent_boot_device)
