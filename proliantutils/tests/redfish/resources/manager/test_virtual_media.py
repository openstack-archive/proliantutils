...
class VirtualMediaUtilsTestCase(testtools.TestCase):
    ...
    def test_get_vm_device(self):
        ...
        manager_obj = manager.HPEManager(
        ...
        obj = virtual_media.get_vm_device(
            'CDROM',
            manager_obj.virtual_media.get_members())

        self.assertIsInstance(obj, virtual_media.VirtualMedia)

    def test_get_vm_device_invalid(self):
        self.conn = mock.Mock()
        with open('proliantutils/tests/redfish/'
                  'json_samples/manager.json', 'r') as f:
            self.conn.get.return_value.json.return_value = json.loads(f.read())
        ...
        self.assertRaisesRegex(
            ...
            virtual_media.get_vm_device, 'XXXX',
            manager_obj.virtual_media.get_members())
