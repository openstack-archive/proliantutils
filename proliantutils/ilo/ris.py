...
class RISOperations(operations.IloOperations):
    ...
    def _rest_op(self, operation, suburi, request_headers, request_body):
        ...
        LOG.debug("[iLO %(host)s] %(operation)s %(url)s "
        ...
        while redir_count:
            ...
            try:
                ...
                LOG.exception("[iLO %(host)s] An error occured while "
                              "contacting iLO. Error: %(error)s",
            ...
            if response.status_code == 301 and 'location' in response.headers:
                ...
                redir_count -= 1
                LOG.debug("[iLO %(host)s] Request redirected to %(url)s.",
                          {'url': url.geturl(), 'host': self.host})
            else:
                break
        else:
            # Redirected for 5th time. Throw error
            msg = ("[iLO %(host)s] URL Redirected 5 times continuously. "
