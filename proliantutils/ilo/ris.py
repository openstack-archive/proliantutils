...
class RISOperations(operations.IloOperations):
    ...
    def _get_iscsi_settings_resource(self, data):
        """Get the iscsi settings resource."""
        try:
    ...
    def _check_iscsi_resource(self, properties=[]):
        """Check if the iscsi resource exists."""

        _, bios_uri, bios_settings = self._check_bios_resource()
    ...
    def _change_iscsi_settings(self, mac, iscsi_info):
        ...
        _, headers, boot_settings = self._get_bios_boot_resource(bios_settings)
        _, headers, map_settings = self._get_bios_mappings_resource(
            bios_settings)
        ...
        if('AuthMethod' in iscsi_info and iscsi_info['AuthMethod'] is 'CHAP'):
            ...
            patch_data['iSCSIBootSources'][0]['iSCSIChapUsername'] = (
                iscsi_info['UserName'])
        self._rest_patch(iscsi_settings_uri, None, patch_data)
    ...
    def set_iscsi_boot_info(self, mac, targetName, lun, ipAddress, port='3260',
                            authMethod=None, username=None, password=None):
        ...
        if(self._validate_uefi_boot_mode() is True):
            iscsi_info = dict()
            iscsi_info['TargetName'] = targetName
            iscsi_info['LUN'] = lun
            iscsi_info['IpAddress'] = ipAddress
            iscsi_info['Port'] = port
            if (authMethod is not None):
                iscsi_info['AuthMethod'] = authMethod
                iscsi_info['UserName'] = username
                iscsi_info['Password'] = password
            self._change_iscsi_settings(mac, iscsi_info)
        else:
