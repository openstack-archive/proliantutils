...
class RISOperations(operations.IloOperations):
    ...
    def _get_vm_device_status(self,  device='FLOPPY'):
        """Returns the given virtual media device status
        ...
        for status, hds, vmed, memberuri in self._get_collection(vmedia_uri):
            ...
            for item in response['MediaTypes']:
                if valid_devices[device] == item.lower():
                    return response
    ...
    def get_vm_status(self, device='FLOPPY'):
        ...
        :raises: IloCommandNotSupportedError, if the command is not supported
                 on the server.
        """
        response = self._get_vm_device_status(device)
    ...
    def set_vm_status(self, device='FLOPPY',
        ...
        :raises: IloCommandNotSupportedError, if the command is not supported
                 on the server.
        """
        # CONNCT is a RIBCL call. There is no such property to set in RIS.
        ...
        if boot_option not in boot_option_map:
            msg = ('Virtualmedia boot option "' + boot_option + '" is not'
                   ' supported on this system.')
            raise exception.IloCommandNotSupportedError(msg)
        ...
        vm_settings = dict()
        vm_settings['Oem'] = dict()
        vm_settings['Oem']['Hp'] = dict()
        vm_settings['Oem']['Hp']['BootOnNextServerReset'] = boot_option_map[boot_option]
