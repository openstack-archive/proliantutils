...
class Ilo_singleton(object):

    class LatestEntryLastOrderedDict(collections.OrderedDict):
       def __setitem__(self, key, value):
           if key in self:
               del self[key]
           super(Ilo_singleton.LatestEntryLastOrderedDict, self).__setitem__(
               key, value)
    ...
    def __init__(self, cls):
        self.cls = cls
        self._instances = Ilo_singleton.LatestEntryLastOrderedDict()
        ...
        if self._if_not_exists(*args):
            with self._lock:
                ...
                if self._if_not_exists(args):
                    ...
                    if len(self._instances) > (self.max_cache_size - 1) :
                        oldest = self._instances.popitem(last=False)
                        LOG.debug("Node cache hit the maximum size."
                            "Removing oldest entry %s"%(oldest[0]))
            ...
            LOG.debug("Using existing object for node %(address)s.",
                      {'address': address})
            # Update the cache
            self._instances.update({address: self._instances[address]})
