    ...
    *proliantutils.ilo.client.SUPPORTED_REDFISH_METHODS*
    ...
    def get_server_capabilities(self):
        ...
        try:
            ...
            capabilities.update(
                {'iscsi_boot': self._get_iscsi_boot_supported(sushy_system)})
            capabilities.update(
    ...
    def _get_iscsi_boot_supported(self, system):
        """Gets iscsi boot support.

        :param system: A HPESystem object
        :returns: 'true' if iscsi boot is supported on the redfish controller
            else 'false'
        """
        supported = 'false'
        try:
            if system.bios_settings.iscsi_settings:
                supported = 'true'
