    ...
    * get_host_power_status(self)
    ...
    def activate_license(self, key):
        ...
        :raises: IloCommandNotSupportedError, if the command is not supported
                 on the server.
        """
        sushy_manager = self._get_sushy_manager(PROLIANT_SYSTEM_ID)

        try:
            lic_uri = (sushy_manager.json['Oem']['Hpe']['Links']
                       ['LicenseService']['@odata.id'])

        except KeyError:
            msg = ('"LicenseService" section in'
                   'Manager/Oem/Hpe/Links does not exist')
