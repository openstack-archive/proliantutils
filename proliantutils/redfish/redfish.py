...
VIRTUAL_MEDIA_MAP = {'FLOPPY': cons.VIRTUAL_MEDIA_FLOPPY,
                     'CDROM': cons.VIRTUAL_MEDIA_CDROM}
...
class RedfishOperations(operations.IloOperations):
    ...
    *proliantutils.ilo.client.SUPPORTED_REDFISH_METHODS*
    ...
    def eject_virtual_media(self, device):
        ...
        try:
            vmedia_device = virtual_media.get_vm_device(
                VIRTUAL_MEDIA_MAP[device],
                manager.virtual_media)
            if not vmedia_device.inserted:
                LOG.info(self._("No media available in the device '%s' to "
