...
def init_and_set_resource_if_not_already(resource_class, resource_id):
    ...
    The name of the attribute which needs to be set has to be returned from
    ...
    if ((not issubclass(resource_class, base.ResourceBase)) or
        ...
        raise TypeError(
            ("'resource_class' and 'resource_id' must be of type ResourceBase "
             "and string/callable respectively. Invalid argument type/s "
             "provided: %(res_class_type)s, %(res_id_type)s") %
...
def lazy_evaluate_for_attribute(instance_attr_name, set_attribute_value=False):
    """Utility decorator to evaluate the attribute value lazily.

    This decorator is to be used to evaluate the attribute so passed
    lazily. It expects the decorated method to have its own logic which
    gets executed on demand when it sees the attribute not being set.
    If the ``set_attribute_value`` value is True then the attribute value is
    set with the result returned by the decorated method.
    :param instance_attr_name: attribute name of the resource
    :param set_attribute_value: If True, then sets the attribute value to
        the result returned by the decorated method. Default value: False
    :returns: the value of attribute
    :raises: AttributeError, if the named attribute does not exist on
        resource instance (the first parameter of decorated method)
    """
    if not isinstance(instance_attr_name, six.string_types):
        raise TypeError(
            ("'instance_attr_name' must be of type string. Invalid argument "
             "type provided: %(attribute_name_type)s") %
            {'attribute_name_type': type(instance_attr_name)})
    ...
    def func_decorator(func):
        ...
        def func_wrapper(this_resource_instance):
            ...
            if instance_attr_val is None:
                ...
                result = func(this_resource_instance)
                if set_attribute_value:
                    setattr(this_resource_instance, instance_attr_name, result)
                return getattr(this_resource_instance, instance_attr_name)
