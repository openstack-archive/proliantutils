...
class HPEUpdateService(base.ResourceBase):
    ...
    from sushy
    """
    fw_update_uri = base.Field(['Actions',
                                '#UpdateService.SimpleUpdate',
                                'target'])

    def get_fw_update_uri(self):
        """Get the firmware uri to activate_license

        :returns: firmware uri
        """
        fw_uri = HPEUpdateService.fw_update_uri._load(self.json, self)
        if not fw_uri:
            raise exception.MissingAttributeError(
                attribute='Actions/#UpdateService.SimpleUpdate/target',
                resource=self.path)
        return fw_uri
    ...
    def flash_firmware_update(self, data):
        ...
        """
        if data is not None:
            target_uri = self.get_fw_update_uri()
