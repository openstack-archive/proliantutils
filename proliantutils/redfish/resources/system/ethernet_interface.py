...
class EthernetInterfaceCollection(base.ResourceCollectionBase):
    ...
    def summary(self):
        ...
        which means the MACs only in 'Enabled' State would be
        returned.
        The returned format will be {<port_id>: <mac_address>}.
        This is because RIBCL returns the data in format
        {'Port1': 'aa:bb:cc:dd:ee:ff'} and ironic ilo drivers inspection
            ...
            for eth in self.get_members():
                if eth.mac_address is not None:
                    if (eth.status is not None and
                            eth.status.health == sys_cons.HEALTH_OK and
                            eth.status.state == sys_cons.HEALTH_STATE_ENABLED):
                        mac_dict.update({str(eth.identity): eth.mac_address})
