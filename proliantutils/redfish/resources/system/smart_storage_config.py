...
class HPESmartStorageConfig(base.ResourceBase):
    ...
    smart_storage_config_message = base.Field(['@Redfish.Settings', 'Messages'])
    ...
    def _generic_format(self, raid_config):
        """Converts the redfish data of current raid configuration to generic
        key-value format to keep in sync with inband cleaning output.
    ...
    def read_raid(self):
        """Gets the current RAID configuration from the system.
        ...
        if result:
            ...
            raid_data['logical_disks'] = self._generic_format(raid_config.json())
            return raid_data
        else:
