...
class HPESystem(system.System):
    ...
    def _get_size_by_smart_storage(self):
        ...
        """
        size = 0
        logical_drive_members = self.array_controllers.logical_drive
        if len(logical_drive_members) > 0:
            for mem in self.array_controllers.logical_drive:
                logical_size = mem.maximum_size
                if size < logical_size:
                    size = logical_size
        else:
            smart_disk_drive_members = self.array_controllers.smart_disk_drive
            if len(smart_disk_drive_members) > 0:
                for mem in self.array_controllers.smart_disk_drive:
