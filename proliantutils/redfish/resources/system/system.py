...
class HPESystem(system.System):
    ...
    def get_smart_storage_config_by_controller_model(self, controller_model):
        """Returns a SmartStorageConfig Instance for controller by model.

        :returns: SmartStorageConfig Instance for controller
        """
        for ac in self.smart_storage.array_controllers.get_members():
            if controller_model == ac.model:
    ...
    def get_controller_model_by_ssc_id(self, ssc_id):
        """Returns a SmartStorageConfig controller model by identity.

        :returns: SmartStorageConfig controller model
        """
        ssc_obj = self.get_smart_storage_config(ssc_id)
        for ac in self.smart_storage.array_controllers.get_members():
            if ac.location == ssc_obj.location:
    ...
    def read_raid(self, raid_config=None):
        ...
        :raises: IloError, on an error from iLO.
        :returns: A list of touple containing controller and respective logical
                  drives
        """
        ...
        if raid_config is None:
            ...
            for ssc_id in self.smart_storage_config_identities:
                try:
                    controller = self.get_controller_model_by_ssc_id(ssc_id)
                    ssc_obj = self.get_smart_storage_config(ssc_id)
                    result.append((controller, ssc_obj.read_raid()))
            ...
            for controller in controllers:
                try:
                    ssc_obj = (
                        self.get_smart_storage_config_by_controller_model(
                            controller))
                    result.append((controller, ssc_obj.read_raid()))
                except exception.IloLogicalDriveNotFoundError as e:
                    ld_exc_count += 1
    ...
    def _parse_raid_config_data(self, raid_config):
        ...
        """
        default_controller = self.get_controller_model_by_ssc_id(
        ...
        for ld in raid_config['logical_disks']:
            if 'controller' not in ld.keys():
                controllers[default_controller].append(ld)
            else:
                ctrl = ld['controller']
    ...
    def get_ssc_controllers_models(self):
        """Return list of models of smart storage array controllers

        :returns: List of models of smart storage array controllers
        """
        models = []
        for ac in self.smart_storage.array_controllers.get_members():
        ...
        for controller in controllers:
            try:
                ...
                if ssc_obj:
                    ...
                    models = self.get_ssc_controllers_models()
