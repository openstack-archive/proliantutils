...
from proliantutils.redfish.resources.system import smartstorage
...
class HPESystem(system.System):
    ...
    def volume(self):
        self._volume = []
        volume_paths = self.storage.volume_paths
    ...
    def smart_storage(self):
        ...
        if path:
            self._smart_storage = smartstorage.SmartStorageCollection(
                self._conn, path, redfish_version=self.redfish_version)
        return self._smart_storage

    @property
    def array_controller(self):
        members = self.smart_storage.get_members()
        for member in members:
            self._array_controller_path = member.array_controller
        path = self._array_controller_path
        if path:
            self._array_controller = (
    ...
    def storage_summary(self):
        ...
        if self._simple_storage_path and not self._volume:
            if size < self.simplestorage.size:
                size = self.simplestorage.size
