...
class HPESystem(system.System):
    ...
    def get_smart_storage_controllers(self):
        ...
        ssd = storage_map.MEDIA_TYPE_MAP_REV[storage_const.MEDIA_TYPE_SSD]
        try:
            models = (self.smart_storage.array_controllers.
                      get_all_controllers_model())
            for model in models:
                controller_obj = (self.smart_storage.array_controllers.
                                  array_controller_by_model(model))
                disk_status = {
                    hdd: controller_obj.physical_drives.has_hdd,
                    ssd: controller_obj.physical_drives.has_ssd
                    }
                controllers[model] = disk_status
            return controllers
        except sushy.exceptions.SushyError as e:
            msg = ('The Redfish controller failed to get smart storage '
                   'controller information. Error: %(error)s'
                   % {'error': str(e)})
            raise exception.IloError(msg)
    ...
    def do_disk_erase(self, controller, disk_type, pattern):
        ...
        try:
            if controller.split('Array ')[1][0] != 'S':
                ...
                ssc_obj = self._get_smart_storage_config_by_controller_model(
                    controller)
                if disk_type == hdd:
                    disks = controller_obj.physical_drives.get_all_hdd_drives()
                else:
                    disks = controller_obj.physical_drives.get_all_ssd_drives()
                ssc_obj.disk_erase(disks, disk_type, pattern)
            else:
                LOG.warn("S-Series smart array controller doesn't support "
                         "sanitize disk erase")
        except sushy.exceptions.SushyError as e:
            msg = ('The Redfish controller failed to perform the sanitize '
                   'disk erase on controller: %(controller)s, on disk_type: '
                   '%(disk_type)s with error: %(error)s'
                   % {'controller': controller, 'disk_type': disk_type,
                      'error': str(e)})
            raise exception.IloError(msg)
