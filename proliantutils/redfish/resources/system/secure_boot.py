...
class SecureBootResource(base.ResourceBase):
    ...
    secure_boot_current_field = base.Field("SecureBootCurrentBoot",
                                     adapter=SECURE_BOOT_CURRENT_BOOT_MAP.get)
    secure_boot_enable_field = base.Field("SecureBootEnable",
    ...
    @property
    def secure_boot_current(self):
        return SecureBootResource.secure_boot_current_field._load(
            self.json, self)

    @property
    def secure_boot_enable(self):
        return SecureBootResource.secure_boot_enable_field._load(
            self.json, self)
