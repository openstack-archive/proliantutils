...
class ArrayControllerCollection(base.ResourceCollectionBase):
    ...
    _max_logical_drive_size = None
    _maximum_disk_size = None
    ...
    @property
    def maximum_logical_drive_size(self):
        """Gets the biggest logical drive

        :returns the size in Bytes.
        """
        if self._max_logical_drive_size is None:
            self._max_logical_drive_size = 0
            logical_drive_members = self.logical_drive
            if len(logical_drive_members) > 0:
                for mem in self.logical_drive:
                    size = mem.maximum_size
                    if self._max_logical_drive_size < size:
                        self._max_logical_drive_size = size
        return self._max_logical_drive_size
    ...
    def maximum_disk_size(self):
        ...
        if self._maximum_disk_size is None:
            ...
            disk_members = self.smart_disk_drive
            if len(disk_members) > 0:
                for mem in disk_members:
                    disk_size = mem.maximum_size
                    if self._maximum_disk_size < disk_size:
                        self._maximum_disk_size = disk_size
            unconfigured_mem = self.unconfigured_disk_drive
            if len(unconfigured_mem) > 0:
                for mem in unconfigured_mem:
                    unconfigured_size = mem.maximum_size
                if self._maximum_disk_size < unconfigured_size:
                    self._maximum_disk_size = unconfigured_size
    ...
    def refresh(self):
        ...
        self._max_logical_drive_size = None
