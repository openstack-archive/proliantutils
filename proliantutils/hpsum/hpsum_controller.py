...
def _parse_hpsum_ouput(exit_code):
    ...
    if exit_code in (0, 1, 253):
        if os.path.exists(OUTPUT_FILE):
            ...
            result = ("SUMMARY: %(return_code)s STATUS of updated components:"
                      " TOTAL: %(total)s SUCCESS: %(success)s" %
                      {'return_code': EXIT_CODE_TO_STRING.get(exit_code),
                       'total': (success + failed), 'success': success})
            if failed != 0:
                result = result + " FAILED: %s" % failed
            return result
...
def _get_clean_arg_value(node, key):
    ...
    :param key: A key to search for in the node (a nested dict).
    :returns: A string value.
    """
    return node['clean_step']['args']['firmware_images'][0].get(key)
