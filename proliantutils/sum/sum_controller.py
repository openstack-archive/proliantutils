...
def _execute_sum(hpsum_file_path, components=None):
    ...
    try:
        ...
        script = os.path.join(cwd, 'run_sum_update.sh')
        processutils.execute('/bin/bash', script, hpsum_file_path, cmd)
    except processutils.ProcessExecutionError as e:
        result = _parse_sum_ouput(e.exit_code)
        if result:
            return result
        else:
            msg = ("Unable to perform SUM based firmware update on the node. "
                   "Error: " + str(e))
...
def update_firmware(node):
    ...
    try:
        ...
        result = _execute_sum(hpsum_file_path, components=components)
